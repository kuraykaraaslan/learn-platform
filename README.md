...
| `npm test` | Vitest. |
| `npm run brand:assets` | Re-render favicon / app icon / OG card from the logo geometry. |
...
modules/
  shared/ui/
    ...
    Logo.tsx                   ← brand mark + wordmark (every chrome renders this)
    logo.geometry.ts           ← the mark's paths, shared with the asset script
    ThemeToggle.tsx
...
app/
  ...
  icon.svg / favicon.ico / apple-icon.png / opengraph-image.png
                               ← generated by scripts/generate-brand-assets.ts
  manifest.ts                  ← PWA manifest (icons live in public/)
