...
For your SaaS, you likely want different algorithms for different endpoints. Public auth endpoints (login, register) benefit from sliding window — no edge bursting. API endpoints used by paying tenants should use token bucket — allow short bursts for legitimate batch operations without penalizing normal usage. Background job triggers benefit from leaky bucket to protect your DB from write floods.
