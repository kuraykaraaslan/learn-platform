...
- **Most "modals" should be in-window `Modal`s from your design system, not separate OS windows** — a true modal child window is the rare case; a modeless secondary window is the common one when multiple windows exist at all
...
- **Building a native modal child window for an in-content confirmation** — heavier than necessary and visually inconsistent with the rest of the app; an in-window `Modal` handles the vast majority of "modal" needs.
