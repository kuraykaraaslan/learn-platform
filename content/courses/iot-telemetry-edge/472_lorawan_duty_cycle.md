...
The law is the **duty cycle**. In the EU868 band a device may occupy its channel for only a small fraction of the time, and that fraction is set by regulation rather than by the network operator. It is enforced as a required silence *after each transmission*: send for 1.5 seconds at 1% and the radio must then stay quiet for about 147 seconds — silence the firmware has to wait out without blocking its loop, which is Lesson 546's subject. This is not a rate limit you can negotiate, retry past, or buy your way out of. **A retry policy written for HTTP is not legal here.**
