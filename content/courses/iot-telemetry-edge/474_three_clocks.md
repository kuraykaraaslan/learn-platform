...
Validation is where the judgement lives. A device clock **ahead of** ingest time is always wrong, because nothing can have measured something that has not happened yet. A device clock far **behind** is ambiguous: it may be an unset clock — a device powers up counting ticks from zero and knows no date at all until told (#549) — or it may be a perfectly good reading that waited three days in a buffer. So the threshold is a policy, it has to be a stated one, and the response is to **quarantine rather than discard** — a device with a broken clock is still reporting something real, and the readings become recoverable once the offset is known.
