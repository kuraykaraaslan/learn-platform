...
**Blue-green deployment** keeps two identical production environments ("blue" and "green") at all times. One is live (serving all traffic), one is idle. You deploy the new version to the idle environment, run smoke tests, then flip the load balancer to send all traffic to the new environment. The old one stays up and idle. If anything goes wrong, the rollback is instant: flip the load balancer back. The trade-off is cost — you pay for two full environments, and you need your database schema changes to be backward-compatible with both versions simultaneously. A field device runs the same play with two flash slots and a single commit write in place of the load balancer (#552) — the same idea at the scale where the flip has to survive losing power halfway through it.
