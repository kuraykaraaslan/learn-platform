...
The crosswalk has one row per (canonical asset, system, external id), and it is not a one-time mapping. Ids change: a GIS re-survey renumbers features, a SCADA re-tag moves `RTU-06` to `RTU-07`, an estate is physically re-tagged and every old identifier needs a recorded successor (#580), a model re-export regenerates GlobalIds (Lesson 490 in the twin course). So each row carries **validity time** — `valid_from` and `valid_to` — and "resolve this id" always means "resolve it *as of* a moment", defaulting to now. That temporal column is what makes Lesson 520's historical replay possible.
